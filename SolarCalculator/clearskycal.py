...
class calsolar:
    ...
    def cal_dzna(df, lat, lon):
        phi = lat
        doy_list = df.index.dayofyear
        hour_list = df.index.hour
        minute_list = df.index.minute
        time_vector = hour_list + minute_list / 60
        B = (360/365)*(doy_list - 81)
        LSTM = 15*7
        EoT = 9.87*np.sin(2*B*np.pi/180)-7.53*np.cos(B*np.pi/180)-1.5*np.sin(B*np.pi/180)
        TC = 4*(lon-LSTM)+EoT
        LST = time_vector + TC/60
        HRA = 15*abs(LST-12)
        delta= -23.45*np.cos(2*np.pi*(doy_list+10)/365)
        zna = np.sin(delta*np.pi/180)*np.sin(phi*np.pi/180) + np.cos(delta*np.pi/180)*np.cos(phi*np.pi/180)*np.cos(HRA*np.pi/180)
        zna = np.arccos(zna)*180/np.pi
        return zna
    ...
    def cal_dairmass(df, lat, lon):
        phi = lat
        doy_list = df.index.dayofyear
        hour_list = df.index.hour
        minute_list = df.index.minute
        
        time_vector = hour_list + minute_list / 60  # cal solar hour
        B = (360/365)*(doy_list - 81)
        LSTM = 15*7
        EoT = 9.87*np.sin(2*B*np.pi/180)-7.53*np.cos(B*np.pi/180)-1.5*np.sin(B*np.pi/180)
        TC = 4*(lon-LSTM)+EoT
    ...
    def cal_dclearsky(df, lat, lon, alt):
        phi = lat
        doy_list = df.index.dayofyear
        hour_list = df.index.hour
        minute_list = df.index.minute
        ...
        Isc = 1366.1
        AM = 1 / (x + 0.50572 * (96.07995 - np.degrees(np.arccos(x))) ** -1.6364)
        iclr = a1 * Isc * x * np.e ** (-a2 * AM * (f1 + f2 * (TL - 1)))
        return iclr 
